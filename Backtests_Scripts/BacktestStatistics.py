...
class BacktestProfile:
    ...
    def show_perf(self, title):
        ...
        ax.plot(df['prediction'] , label = 'Prediction BPF Price' ,linewidth=1.5, color='orange', alpha = 0.9, zorder=1)
        ax.scatter(df.index , df['long_open'], label = 'Open long' , marker = '^', color = 'green',alpha =1,s = 200, zorder=2) 
        ax.scatter(df.index , df['short_open'], label = 'Open short' , marker = 'v', color = 'blue',alpha =1, s = 200, zorder=2)
        ax.scatter(df.index , df['long_short_close'], label = 'Close position' , marker = 'x', color = 'red',alpha =1, s = 200, zorder=2)
        ax.set_title('BPF Price History with buy and sell signals', fontsize=18)
